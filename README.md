# She Can Foundation Assignment

A landing page for an NGO dedicated to
empowering women through education, skill development,
and community support.

---

## 🔗 Live Website
[Click here to view live page](https://vaishuuus.github.io/She_Can_Foundation_Frontend/)

---

## Video Demo:
Shared as mp4 file

---

## 📸 Preview
<img width="959" height="479" alt="Screenshot 2026-06-05 182222" src="https://github.com/user-attachments/assets/f56e1b72-d71b-473a-9576-f3555c2b4c6b" />
<img width="959" height="479" alt="Screenshot 2026-06-05 182222" src="https://github.com/user-attachments/assets/fdd9c33a-fc43-4c37-8870-3988e68f4049" />
<img width="959" height="476" alt="Screenshot 2026-06-05 182249" src="https://github.com/user-attachments/assets/1de02b2a-b95f-4ada-9ffa-dbd79616d704" />



---

##  Built With
- HTML5
- CSS3
- CSS Transitions and Hover Effects
- Codepen.io

---

## Features
- NGO navbar with gold branding
- Hero section with elegant tagline
- About section with image and description
- 3D hover effect on image
- Gold glow hover effect on button
- Dark navy, gold and white colour theme
- Formal Times New Roman typography

---

## 📁 Files
| File | Purpose |
|------|---------|
| index.html | Page structure |
| style.css | All styling |
| preview.png | Screenshot of webpage |
